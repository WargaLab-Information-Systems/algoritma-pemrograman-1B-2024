def tambah_karyawan():
    ...
    nama = input(" Nama: ")
    alamat = input("MasukkMasukkanan Alamat: ")
...
def tampilkan_karyawan(karyawan_list):
    if len(karyawan_list) == 0: #menghitung jumlah eleme dalam 1 objek 
...
def cari_karyawan(karyawan_list, index, nilai):
    ditemukan = False
    for karyawan in karyawan_list:
        if karyawan[index].lower() == nilai.lower():  #mengabaikan kapital #urutan data
            print(f"NIP: {karyawan['NIP']}, Nama: {karyawan['Nama']}, Alamat: {karyawan['Alamat']}, Departemen: {karyawan['Departemen']}, Jabatan: {karyawan['Jabatan']}")
            ditemukan = True
    if ditemukan:
        print(f"Tidak ada karyawan dengan {index} '{nilai}'.")
